...
- **ん** = 받침 ㄴ. 칸=かん(간=がん — 청탁 규칙은 받침과 무관하게 유지), 칸이=かんい vs 카니=かに (초성 ㅇ 덕분에 자동 구분 — 로마지 n' 불필요)
...
- **HanguljiCore 유닛테스트가 품질의 핵심.** 테이블 주도 골든 테스트:
  - §4.1 기본표 전수 + 요음 전수
  - 엣지: 칸이/카니, 받침 재해석, 촉음(받침 ㅅ/ㄱ/ㅂ/ㄷ), ん 별칭(ㅇ/ㅁ), 장음 시퀀스, 별칭 정규화(까/쓰/쯔/찌), 띠/뜨, 확장 가타카나 음절, 매핑 불가 음절 통과, 조합 중 백스페이스(자모 단위 복원)
